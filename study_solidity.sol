...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    mapping(address => mapping(address => uint256)) private _allowances;        //A가 C에게 10토큰만큼 권한을 줬을때, _allowances[A][C] = 10
    ...
    function _transfer(
        ...
        _beforeTokenTransfer(from, to, amount); //토큰을 보내기 전에 하고싶은 것들은 자유롭게 적는 함수

        uint256 fromBalance = _balances[from];
        require(fromBalance >= amount, "ERC20: transfer amount exceeds balance");

        //unchecked는 overflow 나 underflow가 일어날 수 있게 하는 기능. solidity가 0.8버전이후 overflow가 발생하면 자동으로 에러를 일으켰지만
        //unchecked로 감싸진 영역에서는 에러발생없이 overflow가 일어나게 됨. unchecked로 가스비 절감 가능
        //unchecked로 감싸준 이유는 위에서 require로 overflow가 안일어나도록 체크를 해줬기 때문에 unchecked를 써서 가스비 절감을 하는 것임.
        unchecked { 
        ...
        _afterTokenTransfer(from, to, amount); //토큰을 보낸 후 할 것들
    }
